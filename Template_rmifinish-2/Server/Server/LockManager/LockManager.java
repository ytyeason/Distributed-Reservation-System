				...
				synchronized (this.lockTable) {
					...
					if (!bConflict) {
						...
						if (bConvert.get(0) == true) {
							...
							TransactionLockObject transactionLockObect1 = new TransactionLockObject(xid, data, TransactionLockObject.LockType.LOCK_READ);
							DataLockObject dataLockObject1 = new DataLockObject(xid, data, DataLockObject.LockType.LOCK_READ);
							
							this.lockTable.remove(transactionLockObect1);
							this.lockTable.remove(dataLockObject1);
		...
		for (int i = 0; i < size; i++)
		{
			...
			{    
				...
				else if (dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
				{
					...
					//TODO: Lock conversion
					if(l_dataLockObject.getLockType() == DataLockObject.LockType.LOCK_READ) {
						for (int j = 0; j<size; j++) {
							DataLockObject d = (DataLockObject)vect.elementAt(j);
							if(d.getXId() != l_dataLockObject.getXId()) {
								System.out.println("someone else aso has read");
								return true;
							}
						}
						bitset.set(0);
					...
					if(l_dataLockObject.getLockType() == DataLockObject.LockType.LOCK_WRITE) {
//						throw new Exception("redundant write");
						throw new RedundantLockRequestException(dataLockObject.getXId(), "redundant WRITE lock request");
//						System.out.println("redundant write");
					}
