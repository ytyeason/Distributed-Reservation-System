...
#MACHINES=(cs-2 cs-6 cs-4 cs-5 cs-9)
MACHINES=(cs-2 cs-6)
...
tmux new-session \; \
	...
	select-pane -t 1 \; \
	send-keys "ssh -t ${MACHINES[0]} \"cd $(pwd) > /dev/null; echo -n 'Connected to '; hostname; ./run_server.sh\"" C-m \; \
	#select-pane -t 2 \; \
	...
	select-pane -t 0 \; \
	send-keys "ssh -t ${MACHINES[1]} \"cd $(pwd) > /dev/null; echo -n 'Connected to '; hostname; sleep .5s; ./run_middleware.sh ${MACHINES[0]}\"" C-m \;
